...
export class GamePageResolveService implements Resolve<boolean|Promise<BehaviorSubject<GameElement>>> {
  ...
  resolve(route: ActivatedRouteSnapshot): boolean|Promise<BehaviorSubject<GameElement>> {
    ...
    this.game = new BehaviorSubject(games[index]);
    ...
    this.game.subscribe(_game => {
      console.log('game updated', _game);
      let _games = this.gameService.games.getValue();
      let i = _games.map(_g => _g.id).indexOf(_game.id);
      if (i === -1) {
        throw new Error('this has already been deleted');
      }
      _games[i] = _game;
